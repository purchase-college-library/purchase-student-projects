<?php
/* ----------------------------------------------------------------------
 * plugins/Contact/controllers/ContactController.php
 * ----------------------------------------------------------------------
 * CollectiveAccess
 ...
 */
	...
	require_once(__CA_LIB_DIR__."/vendor/htmlpurifier/HTMLPurifier.standalone.php");
...
// 			JavascriptLoadManager::register('tokeninput');
 			...
 			if(!is_dir(__CA_APP_DIR__.'/plugins/Contact/themes/'.$this->ops_theme.'/views')) {		// if theme is not defined for this plugin, try to use "default" theme
 				$this->ops_theme = 'default';
 			}
 			parent::__construct($po_request, $po_response, array(__CA_APP_DIR__.'/plugins/Contact/themes/'.$this->ops_theme.'/views'));
 			
			$this->opo_plugin_config = Configuration::load($this->request->getAppConfig()->get('application_plugins').'/Contact/conf/Contact.conf');
 			
 			if (!(bool)$this->opo_plugin_config->get('enabled')) { die(_t('Contact plugin is not enabled')); }
 			
 			MetaTagManager::addLink('stylesheet', $po_request->getBaseUrlPath()."/app/plugins/Contact/themes/".$this->ops_theme."/css/share.css",'text/css');
 		...
 		public function sendEmailObject() {
			...
			if(!$this->request->isLoggedIn()){
 			...
 			if(sizeof($va_errors) == 0){
				...
				ob_start();
				require($this->request->getAppConfig()->get('application_plugins')."/Contact/themes/".$this->ops_theme."/views/mailTemplates/share_object_email_text.tpl");
				$vs_mail_message_text = ob_get_contents();
				ob_end_clean();
				ob_start();
				require($this->request->getAppConfig()->get('application_plugins')."/Contact/themes/".$this->ops_theme."/views/mailTemplates/share_object_email_html.tpl");
				...
				/**
					if($this->opo_plugin_config->get('enable_media_attachment')=="setting this string to keep this iff from being triggered"){
...
// 					$this->response->setRedirect(caNavUrl($this->request, 'Detail', 'Object', 'Show', array('object_id' => $pn_object_id)));
 		...
 		public function sendEmail() {
			...
			if(!$t_item->load($pn_item_id)){
 			...
 			$ps_from_email = $this->request->getParameter('from_email', pString);
 			$ps_from_name = $this->request->getParameter('from_name', pString);
			$ps_subject = "Contact Us form on Digital Archives site from: ".$ps_from_name." ( ".$ps_from_email." )";
// 			$ps_subject = $this->request->getParameter('subject', pString);
 			$ps_message = $this->request->getParameter('message', pString);
 			$pn_security = $this->request->getParameter('security', pInteger);
 			$pn_sum = $this->request->getParameter('sum', pInteger);
...
//    		$ps_subject = $o_purifier->purify($ps_subject);
			...
			if(!$this->request->isLoggedIn()){
 			...
 			if(sizeof($va_errors) == 0){
				# -- generate mail text from template - get both html and text versions
				ob_start();
				require($this->request->getAppConfig()->get('application_plugins')."/Contact/themes/".$this->ops_theme."/views/mailTemplates/share_email_text.tpl");
				$vs_mail_message_text = ob_get_contents();
				ob_end_clean();
				ob_start();
				require($this->request->getAppConfig()->get('application_plugins')."/Contact/themes/".$this->ops_theme."/views/mailTemplates/share_email_html.tpl");
				$vs_mail_message_html = ob_get_contents();
				ob_end_clean();
