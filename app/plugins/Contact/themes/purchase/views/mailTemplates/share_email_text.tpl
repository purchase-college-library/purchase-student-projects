<?php
	print _t("%1 clicked contact us while viewing %2 .  To view the record online visit %3.", $ps_from_name, $this->request->config->get("site_host"), $this->request->config->get("site_host").caNavUrl($this->request, 'Detail', $vs_controller, 'Show', array($t_item->PrimaryKey() => $pn_item_id)))."\n\n";
	if($ps_message){
