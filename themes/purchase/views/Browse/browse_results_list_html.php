	...
	$va_add_to_set_link_info = caGetAddToSetInfo($this->request);
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit() && ($vn_c < $vn_hits_per_block)) {
				...
				$vs_label_detail_link 	= "<b>Title: </b>".caDetailLink($this->request, $qr_res->get("{$vs_table}.preferred_labels"), '', $vs_table, $vn_id);
				if ($qr_res->get('ca_entities.preferred_labels', array('restrictToRelationshipTypes' => array('student'), 'delimiter' => ', '))) {
					$vs_label_detail_link.= "<br/><b>Name: </b>".$qr_res->get('ca_entities.preferred_labels', array('restrictToRelationshipTypes' => array('student'), 'delimiter' => ', '));
				}
				if ($va_major = "<b>Major: </b>".$qr_res->get('ca_occurrences.preferred_labels', array('delimiter' => ', '))) {
					$vs_label_detail_link.= "<br/>".$va_major;
				}								
				$vs_thumbnail = "";
				$vs_type_placeholder = "";
				$vs_typecode = "";
				...
				$vs_rep_id = $qr_res->get("ca_object_representations.representation_id");
				$vs_obj_id = $qr_res->get("ca_objects.object_id");
				$vs_download_link = "";
				if ($vs_rep_id ) {
					$vs_download_link = caNavLink($this->request, '<i style="padding-left:10px;" class="fa fa-download"></i>', '', '', 'Detail', 'DownloadRepresentation', array('representation_id' => $vs_rep_id, 'object_id' => $vs_obj_id, 'download' => 1, 'version' => 'original'));
				}			
	...
	<div class='bResultListItemCol col-xs-{$vn_col_span_xs} col-sm-{$vn_col_span_sm} col-md-{$vn_col_span}'>
		<div class='bResultListItem' onmouseover='jQuery(\"#bResultListItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultListItemExpandedInfo{$vn_id}\").hide();'>
			...
			<div class='bResultListItemExpandedInfo' id='bResultListItemExpandedInfo{$vn_id}'>
				<hr>
				{$vs_expanded_info}{$vs_add_to_set_link}{$vs_download_link}
